    ...
    return <>
        <ul className="overflow-auto">
            {profiles.length > 0 && profiles.map((elem, key) => {
                ...
                return <li key={key}>
                    <Link href={`/chat/${elem.username}`} onClick={()=>setProfile(key)} className={`flex items-center px-3 py-2 text-sm transition duration-300 ease-in-out border-b border-gray-300 cursor-pointer ${active ? "bg-gray-100" : ""} hover:bg-gray-100 focus:outline-none`}>
