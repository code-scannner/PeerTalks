...
export default function ChatLabel({user}){
    const active = false;
    return <Link href={`/chat/${user.username}`} className={`flex items-center px-3 py-2 text-sm transition duration-300 ease-in-out border-b border-gray-300 cursor-pointer ${active ? "bg-gray-100" : ""} hover:bg-gray-100 focus:outline-none`}>
    ...
    <div className="w-full pb-2">
        <div className="flex justify-between">
            <span className="block ml-2 font-semibold text-gray-600">{user.fname}</span>
